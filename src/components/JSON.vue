...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="book in janeBooks" :key="book.title">{{ book.title }} ({{ book.year }})</li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(hour, day) in bookstores.openingHours" :key="day">
          {{ day }} - {{ hour.open }} to {{ hour.close }}
        </li>

        <li>Monday - {{ bookstores.openingHours.weekdays.open }} to {{ bookstores.openingHours.weekdays.close }}</li>
        <li>Tuesday - {{ bookstores.openingHours.weekdays.open }} to {{ bookstores.openingHours.weekdays.close }}</li>
        <li>Wednesday - {{ bookstores.openingHours.weekdays.open }} to {{ bookstores.openingHours.weekdays.close }}</li>
        <li>Thursday - {{ bookstores.openingHours.weekdays.open }} to {{ bookstores.openingHours.weekdays.close }}</li>
        <li>Friday - {{ bookstores.openingHours.weekdays.open }} to {{ bookstores.openingHours.weekdays.close }}</li>
        <li>Saturday - {{ bookstores.openingHours.weekends.open }} to {{ bookstores.openingHours.weekends.close }}</li>
        <li>Sunday - {{ bookstores.openingHours.weekends.open }} to {{ bookstores.openingHours.weekends.close }}</li>
      </ul>
...
const janeBooks = computed(() => {
  const jane = authors.filter((author) => author.name === 'Jane Austen')
  return jane.length > 0 ? jane[0].famousWorks : []
})
</script>
